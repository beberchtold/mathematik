...
var slow;  // true, falls delay>0; falls delay==0, dann kein timer
var timer1;
var first=true;  // einmalig bei run den timer1 stellen
...
window.onload=init;
    ...
    function Start() {   
	   ...
	   document.getElementById("goBttn").value = "Go";
	   document.getElementById("stepBttn").disabled=false;
       document.getElementById("goBttn").onclick = function() {
             if (pause) {
             	pause=false;
             	document.getElementById("goBttn").value = "Stop";
             	doGo();
             }
             else {
             	document.getElementById("goBttn").value = "Go";
    ...
    function run() {
	  if (!pause) {
         ...
         if (slow) timer = setTimeout(function(){step()},delay); 
		   else {if (first) {timer1 = setTimeout(function(){step()},10);}
		           else while (numused<12) step();}
	  }
      if (pause) {
		document.getElementById("label1").firstChild.data ="Pause";
        document.getElementById("reset").disabled=false;		
	  }
    ...
    function step()  {
      ...
      if (!schrittweise) {
        if (slow) {clearTimeout(timer); run();}
		if (first) {first=false; clearTimeout(timer1);run();}
      }	  
    }

    function doStep() {
      schrittweise=true;
	  if (numused==12) success=false;
